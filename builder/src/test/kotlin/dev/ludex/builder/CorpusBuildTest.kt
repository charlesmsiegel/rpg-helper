...
class CorpusBuildTest {
    ...
    fun `every alias is stored in the form the index can match`() {
        db().use { database ->
            ...
            for (alias in aliases) {
                assertEquals(dev.ludex.pack.Utf8.foldForIndex(alias), alias)
            }
        }
    }
    ...
    fun `every alias is stored in the form the rewriter looks it up by`() {
        ...
        db().use { database ->
            for (alias in database.map("SELECT alias FROM entities") { it.string(0) }) {
                assertEquals(dev.ludex.pack.Tokenizer.indexForm(alias), alias)
            }
        }
    }
    ...
    fun `a setting parent's expansions are generated from its redacted prose`() {
        ...
        db().use { database ->
            ...
            assertEquals(
                fromRedacted.map {
                    dev.ludex.pack.Float16.encodeVector(CorpusPack.EMBEDDER.embed(it)).toList()
                },
                stored,
            )
        }
