...
import dev.ludex.session.BUNDLED
import dev.ludex.session.EMBEDDER
import dev.ludex.session.GATES
import dev.ludex.session.Library
import dev.ludex.session.Rules
import dev.ludex.session.Store
...
ludex — build packs, and answer questions out of them.
    ...
    Store(Path.of(arguments[0])).use { store ->
        ...
        return when (val result = store.library.install(source, confirmReplacing = existing)) {
            is InstallResult.Installed -> {
                ...
                if (dev.ludex.pack.BuildReport.shipsUnchecked(result.buildNotes)) {
                    println(
...
private fun printCheck(store: Store, documents: DocumentStore, documentId: Long) {
    ...
    check.flagged.forEach { flagged ->
        ...
        flagged.citation?.let { println("     — ${dev.ludex.routing.render(it)}") }
...
private fun roll(arguments: List<String>): Int {
    ...
    Library.open(listOf(Path.of(arguments[0]))).use { library ->
        ...
        val citation = PackCitations(library.packs)
            .resolve(dev.ludex.pack.ChunkRef(library.packs.single().packUid, result.chunkId))
        ...
        citation?.let { println("    — ${dev.ludex.routing.render(it)}") }
        return 0
    }
}
